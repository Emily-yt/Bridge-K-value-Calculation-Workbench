...
- **系统设置**：定期巡检天数、提前提醒天数、最低正常 K 值配置、用户权限管理
- **用户权限**：计算员与管理员角色区分
...
| K 值范围 | 判定结果 | 颜色标识 | 建议措施 |
|---------|---------|---------|---------|
| K ≥ 2.0 | 安全充裕 | 🟢 绿色 | 承载能力充裕 |
| 1.5 ≤ K < 2.0 | 满足运营 | 🔵 蓝色 | 加强日常监测 |
| 1.0 ≤ K < 1.5 | 承载偏低 | 🟡 黄色 | 限制通行荷载，尽快安排加固 |
| K < 1.0 | 不满足运营 | 🔴 红色 | 立即限制通行，安排专项检测 |
...
| 菜单项 | 图标 | 功能说明 | 权限 |
|--------|------|----------|------|
| 首页 | 🏠 | 统计概览、快捷入口、待办提醒 | 全部 |
| K值计算 | 🧮 | 桥梁列表、详情、计算（核心功能） | 全部 |
| 统计分析 | 📊 | K值分布、趋势图表、预警列表 | 全部 |
| 系统设置 | ⚙️ | 参数配置、用户管理 | 管理员 |
